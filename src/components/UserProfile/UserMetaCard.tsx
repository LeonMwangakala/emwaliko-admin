...
export default function UserMetaCard() {
  ...
  const { profile, loading, error, updateProfilePicture } = useProfile();
  ...
  const handleRemovePicture = async () => {
    ...
    // In a real implementation, you might want to add a "remove picture" API endpoint
    closeModal();
  };
  ...
  const getProfilePictureUrl = () => {
    if (profile?.profile_picture) {
      ...
      return `${baseUrl}/storage/${profile.profile_picture}`;
    }
          ...
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            <div className="w-20 h-20 overflow-hidden border border-gray-200 rounded-full dark:border-gray-800 flex items-center justify-center bg-gray-100 dark:bg-gray-800">
              {profilePictureUrl ? (
                <img 
                  ...
                  onError={(e) => {
                    // Hide the image and show icon if it fails to load
              ...
              {profilePictureUrl && (
                <svg 
                  className="w-8 h-8 text-gray-400 dark:text-gray-500 hidden" 
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
                >
            ...
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              <button
                ...
                title={profilePictureUrl ? "Change Profile Picture" : "Upload Profile Picture"}
              >
                <svg
